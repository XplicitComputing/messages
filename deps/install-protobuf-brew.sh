...
# Check for existence of tar file and correctness.
cat <<- _end_text > protobuf.md5
cc4f50740430d9488312e5e48bc94d68  protobuf-all-3.8.0.tar.gz
_end_text
#md5sum -c protobuf.md5
# Eventually, use compare the sums; for now, openssl|awk simply succeeds.
openssl md5 protobuf.md5 | awk '{print $2}' 
rc_md5=$?
if [ ! -f protobuf-all-3.8.0.tar.gz ]; then
    wget https://github.com/protocolbuffers/protobuf/releases/download/v${TAG}/protobuf-all-${TAG}.tar.gz
    # perhaps check MD5 sum in a limited loop
fi
